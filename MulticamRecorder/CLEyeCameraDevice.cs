...
namespace CLEyeMulticam
{
    ...
    public class CLEyeCameraDevice : DependencyObject, IDisposable, ICamera
    {
        ...
        public void Create() {
            if (UUID != null && !UUID.Equals(Guid.Empty))
            {
                _valid = Create(UUID);
            }
            else {
                _valid = Create(CLEyeCameraDevice.CameraUUID(idx));
            }

            if (_valid)
                log.Debug("Camera created: " + UUID);
            else
                log.Debug("Camera cannot be created");
        }
        ...
        private void InitBitmap()
        {
            ...
            // Invoke event
            log.Debug("Bitmap Source created");
